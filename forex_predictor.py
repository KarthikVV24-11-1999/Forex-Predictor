...
dataframe.fillna(-0.19,inplace=True)
# Filling NaN with mean in testing dataframe
testframe .isnull().any().sum()
testframe.fillna(0.55,inplace=True)
...
classifier = svm.SVC(C=4,kernel='sigmoid',gamma = "scale", coef0=0.192)
...
v = np.var(target_array)
pre = [1]*len(predicted)
for i in range(0,len(predicted)):
    pre[i] = predicted[i] - 0.998630
    if predicted[i] <= 0.999600 and predicted[i] >= 0.997660:
        predicted[i]= 0.998630 + v*9.3
    elif predicted[i] >= 0.999600:
        predicted[i]= predicted[i] + pre[i]/3.3
    elif predicted[i] <= 0.997660:
        predicted[i]= predicted[i] + pre[i]/1.5
distances = np.array(pre)
pd.DataFrame(distances).to_csv("distances_1.csv")
